...
# Hakan Bostan
# 
# In this file we need again implement IADDQ to make add operations more efficient
# and not to waste Cycles on moving 1 to register and adding each time. So the same
# modifications as the seq-full.hcl is done in this file.
#
#
# iaddq $constant, %register
#
# To implement iaddq we first need to declare that IADDQ is indeed
# a valid instruction. So we add the IIADDQ to the list of instructions
# that make bool instr_valid true. (Line: 201)
#
# Since IADDQ requires a register id, we also need to add IIADDQ to list
# that makes bool need_regids true. (Line: 214)
#
# It also requires a constant so we also need to make bool need_valC true
# thus we add IIADDQ to that list also (Line: 218)
#
# IADDQ takes register id as the second parameter (i.e. rB in context) so
# we add IIADDQ to the list which makes word srcB take the value rB.(Line: 238)
#
# Also the result is stored in the register that is given to IADDQ, and
# the given register is given as rB, so the destination should also be rB.
# Therefore, we add IIADDQ to the list which makes word dstE take the value rB.(Line: 245)
#
# This concludes Decode stage. Now: Execute Stage
#
# word aluA should be our constant value so we want to make it valC, so we add
# IIADDQ to corresponging list. (Line: 282)
#
# word aluB should be rB so we add IIADDQ to corresponding list.(Line: 291)
#
# Since ALU's default behaviour is ALUADD (i.e. addition we dont need to do anything)
#
# After the operation we need to update the condition codes, just like any IOPQ operation.
# So IIADDQ is added to make bool set_cc true. (Line: 303)
#
#
####################################################################
